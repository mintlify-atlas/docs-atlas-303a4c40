---
title: "DeepSeek Harness Architecture: Everything is a Plugin"
sidebarTitle: "Overview"
description: "Understand how DeepSeek Harness composes a plugin tree from profiles and bundles, its core package responsibilities, and where new behavior attaches in the system."
---

DeepSeek Harness (dsh) is an open-source, plugin-first AI agent framework. Every part of the product — model adapter, tool registry, session log, and agent loop — is a plugin mounted on a shared context. Because all parts are plugins, all parts are replaceable from configuration: there is no privileged core to patch.
...
Here are the core packages that contribute to the Cordis tree.
...
The [event map](/architecture/agent-lifecycle) lists how producers and consumers relate for every major event in the system.
...
<CardGroup cols={2}>
  <Card title="Agent Lifecycle" icon="arrows-rotate" href="/architecture/agent-lifecycle">
    How agents start, process turns and steps, handle cancellation, and reach quiescence.
  </Card>
  <Card title="Session Log" icon="database" href="/architecture/session-log">
    The append-only, event-sourced source of truth for all agent interactions.
  </Card>
</CardGroup>
